...
const createArticle = async (data) => {
  ...
  if (existing) throw new AppError('An article with this URL already exists', 409);
  ...
  return articleRepository.create({ ...data, addedBy: 'admin' });
};
...
const getArticles = async ({ page = 1, limit = 10, sortBy = '-publishedAt', onlyApproved = true }) => {
  ...
  const [articles, total] = await Promise.all([
    articleRepository.findWithFilters({
      ...
      limit: Number(limit),
      sortBy,
      projection: '-content', // list view excludes full body text
    }),
...
const approveArticle = async (id) => {
  const article = await articleRepository.updateById(id, { isApproved: true });
  if (!article) throw new AppError('Article not found', 404);
  return article;
};
...
const buildSourceFilter = async (sourceParam) => {
  ...
  if (isObjectId) return { source: sourceParam };

  const matches = await sourceRepository.findAll({ name: new RegExp(sourceParam, 'i') });
  return { source: { $in: matches.map((s) => s._id) } };
};

const searchArticles = async (params, userId = null) => {
  const {
    q, title, author, source, category, country, language,
    dateFrom, dateTo, page = 1, limit = 10, sortBy = '-publishedAt',
  } = params;
  ...
  if (category) {
    const categoryDoc = await categoryRepository.findBySlug(category.toLowerCase());
    if (!categoryDoc) {
      return { articles: [], pagination: { total: 0, page: Number(page), limit: Number(limit), totalPages: 0 } };
    }
  ...
  const [articles, total] = await Promise.all([
    articleRepository.searchArticles({
      filter, textQuery: q, page: Number(page), limit: Number(limit), sortBy, projection: '-content',
    }),
    articleRepository.countSearchResults(filter, q),
  ]);

  // Fire-and-forget logging — a failed history log should never break the actual search
  if (q) {
    searchHistoryRepository
      .logSearch({ query: q, user: userId, resultsCount: total })
